...
//         { name: 'Publication', value: `${res.data[0].publishedAt}` },
...
//           console.log(member)
//           client.users.fetch(`${member.discord_user_id}`, false).then((user) => {
//             user.send(latestNews);
//           });
//         })
//       })
//       .catch((err) => {
          ...
          if(res.data == "deleted"){
            subscribed
              ...
              .addFields(
                ...
                { name: '\u200B', value: '\u200B' },
                { name: `${name}  (${discord_user_id}) has unsubscribed!`, value: `\u200B`},
    ...
    axios
      ...
      .then((res) => {
        res.data.forEach(member => {
          console.log(member)
          client.users.fetch(`${member.discord_user_id}`, false).then((user) => {
            user.send('This is a test message');
          });
        })
      })
      .catch((err) => {
